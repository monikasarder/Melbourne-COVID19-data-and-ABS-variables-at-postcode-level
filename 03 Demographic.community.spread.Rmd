...
pop.poa <- all %>% select(Postcode, Suburb, Population, Cases.per.100K, `Confirmed cases (ever)` )

#Set small pop postcodes to 0
pop.poa$Cases.per.100K<-ifelse(pop.poa$Population <1500, 0, pop.poa$Cases.per.100K)

#Set postcodes with recent estates to 0
...
dat <- st_set_geometry(all, NULL)

dat <-dat %>% filter(!Population <1500, !Suburb %in% c("Plenty","Somerton","Ardeer, Deer Park East"),
                     !Postcode %in% c("3062","3010"))


main.table<- dat %>%
  select(Postcode, Suburb,`Confirmed cases (ever)`, Population, Cases.per.100K) %>%
...
standard <-c('Postcode','Exclude','Confirmed cases (ever)', 'Population','Cases.per.100K')
