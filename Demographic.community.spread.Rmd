...
library(absmapsdata)
library(viridis)
datm <- readRDS("Blog.Melbourne.Postcodes.wrangled.RDS")
...
knitr::opts_chunk$set(collapse = TRUE, out.width = "100%")
datvis <-readRDS("Blog4.Case.Numbers.RDS")

pop.poa <- datvis %>% select(Postcode, Suburb, Tot_P_P, Cases.per.100K, Cumulative.cases)
...
geospatial.plot <-datvis %>%
  plot_geo(split = ~Postcode, showlegend = FALSE, hoverinfo = "text", color = ~Cases.per.100K, 
           ...
           text = ~paste("Postcode:", Postcode, 
                         ...
                         "<br>","All cases 6 Aug:", Cumulative.cases
                         )) %>%
  add_sf() %>%
  layout(geo = g)
...
main.table<- dat %>%
  select(Postcode, Suburb,  Cumulative.cases, Tot_P_P, Cases.per.100K) %>%
...
A positive correlation was identified between the percentage of residents whose personal income is less than $500 per week and COVID19 cases, with a correlation of 0.176 (p-value = 0.008).  
...
A positive correlation was also identified between the percentage of Postcode residents who lived in rental properties and the level of COVID19 cases, with a correlation of 0.38 (p-value <0.000).   
...
dat <- dat %>%
  mutate(Percentage.500 = (P_Neg_Nil_income_Tot +P_1_149_Tot + P_150_299_Tot +P_300_399_Tot+
                             P_400_499_Tot)/(P_Tot_Bracket - P_PI_NS_ns_Tot))

dat <-dat %>% mutate(Percentage.500 = round(Percentage.500*100, digits = 2))
```

```{r, echo = FALSE, warnings = FALSE}
fit.perc = lm(Cases.per.100K ~ Percentage.500, data = dat)
p1 <- dat %>%
  plot_ly(x = ~Percentage.500, y = ~Cases.per.100K,
        hoverinfo = "text",
        text = ~paste("Postcode:" ,Postcode,"<br>",
                      "Suburb:" ,Suburb,"<br>",
                     "Cases:",Cases.per.100K,"<br>",
                     "Percentage under 500:",Percentage.500)) %>%
  add_markers(name = ~ "(%) Under $500 ") %>%
  add_lines(x = ~Percentage.500, y = ~ fitted(fit.perc),  showlegend = F)



dat <-dat %>% mutate(Percentage.rent = round((R_Tot_Total/Total_Total)*100, digits = 2))

fit.rent = lm(Cases.per.100K ~ Percentage.rent, data = dat)

p2 <- dat %>%
  plot_ly(x = ~Percentage.rent, y = ~Cases.per.100K,
        hoverinfo = "text",
        text = ~paste("Postcode:" ,Postcode,"<br>",
                      "Suburb:" ,Suburb,"<br>",
                     "Cases:",Cases.per.100K,"<br>",
                     "(%) Rental:", Percentage.rent)) %>%
  add_markers(name = ~ "(%) Rental") %>%
  add_lines(x = ~Percentage.rent, y = ~ fitted(fit.rent), showlegend = F)
...
correlation.dwelling <-read.csv("Blog.correlation.dwelling.csv")

kable(correlation.dwelling)%>%
...
dat <- dat %>% mutate(House = round(OPDs_Separate_house_Persons/Total_PDs_Persons*100, digits = 2))

dat <- dat %>% mutate(Semi.detached = round(OPDs_SD_r_t_h_th_Tot_Psns/Total_PDs_Persons*100, digits = 2))
dat <- dat %>% mutate(Flat = round(OPDs_Flt_apart_Tot_Psns/Total_PDs_Persons*100, digits = 2))

fit.det <- lm(Cases.per.100K ~ Semi.detached, data = dat)

p1 <-dat %>%
 plot_ly(x = ~Semi.detached, y = ~Cases.per.100K,
        hoverinfo = "text",
        text = ~paste("Postcode:" ,Postcode,"<br>",
                      "Suburb:" ,Suburb,"<br>",
                     "Cases:",Cases.per.100K,"<br>",
  ...
  add_lines(x = ~ Semi.detached, y = ~fitted(fit.det), showlegend =  FALSE)

fit.house <- lm(Cases.per.100K ~ House, data = dat)

p2 <-dat %>%
plot_ly(x = ~House, y = ~Cases.per.100K,
        hoverinfo = "text",
        text = ~paste("Postcode:" ,Postcode,"<br>",
                      "Suburb:" ,Suburb,"<br>",
                     "Cases:",Cases.per.100K,"<br>",
                     "(%) House:",House)) %>%
  add_markers( showlegend =  FALSE) %>%
  add_lines(x = ~ House, y = ~fitted(fit.house), showlegend =  FALSE)

fit.flat <- lm(Cases.per.100K ~ Flat, data = dat)

p3 <-dat %>%
plot_ly(x = ~Flat, y = ~Cases.per.100K,
        hoverinfo = "text",
        text = ~paste("Postcode:" ,Postcode,"<br>",
                      "Suburb:" ,Suburb,"<br>",
                     "Cases:",Cases.per.100K,"<br>",
                     "(%) Flat:",Flat)) %>%
  add_markers( showlegend =  FALSE) %>%
  add_lines(x = ~ Flat, y = ~fitted(fit.flat), showlegend =  FALSE)

plotly::subplot(p1, p2, p3, nrows = 2, margin = 0.07)%>%
layout(
  yaxis = list(title = 'Cases per 100K'),
  yaxis3 = list(title = 'Cases per 100K'),
  xaxis = list(title = '(%) Semi-detached'),
  xaxis2 = list(title = '(%) House'),
...
At the peak of the second wave, there was a positive correlation between the number of Postcode residents who did not speak English at home and infection numbers, measured at 0.424 (p-value <0.000). Significant resources have now been directed towards resource translation, as well as going door to door.

There was also positive correlation of 0.277 (p-value <0.000), between the proportion of households with no internet access, and COVID19. The digital divide may be posing a significant barrier to compliance with safety measures.

**Figure 4 COVID19 against Home language, Internet access - by Postcode**
```{r, echo = FALSE, warnings = FALSE}


dat <- dat %>% mutate(Lang.NE = round(Lang.NE*100, digits = 2))
...
dat <- dat %>% mutate(No.internet = round(I_NA_Total/Total.Internet*100, digits = 2))


fit.internet = lm(Cases.per.100K ~ No.internet, data = dat)

p2 <- dat %>%
  plot_ly(x = ~No.internet, y = ~Cases.per.100K,
        hoverinfo = "text",
        text = ~paste("Postcode:" ,Postcode,"<br>",
                      "Suburb:" ,Suburb,"<br>",
                     "Cases:",Cases.per.100K,"<br>",
                     "(%) No internet households:", No.internet)) %>%
  add_markers(name = ~ "(%) Households no internet") %>%
  add_lines(x = ~No.internet, y = ~ fitted(fit.internet), showlegend = F)
...
dat <- dat %>% mutate(Transport.prop = round(Transport.prop*100, digits = 2))

fit.transport = lm(Cases.per.100K ~ Transport.prop, data = dat)

p2 <- dat %>%
  plot_ly(x = ~ Transport.prop, y = ~Cases.per.100K,
        hoverinfo = "text",
        text = ~paste("Postcode:" ,Postcode,"<br>",
                      "Suburb:" ,Suburb,"<br>",
                     "Cases:",Cases.per.100K,"<br>",
                     "(%) Employed transport:", Transport.prop)) %>%
  add_markers(name = ~ "(%) Employed transport") %>%
  add_lines(x = ~Transport.prop, y = ~ fitted(fit.transport), showlegend = F)
...
# Data downloads 
```{r, echo = FALSE, warning = FALSE}
xfun::embed_files('Blog.income.csv', text = "Income data")
```
\
```{r, echo = FALSE, warning = FALSE}
xfun::embed_files('Blog.tenure.csv', text = "Rental data")
```
\
```{r, echo = FALSE, warning = FALSE}
xfun::embed_files('Blog.language.csv', text = "Home language data")
```
\
```{r, echo = FALSE, warning = FALSE}
xfun::embed_files('Blog.internet.csv', text = "Internet connection data")
```
\
```{r, echo = FALSE, warning = FALSE}
xfun::embed_files('Blog.dwelling.csv', text = "Dwelling type data")
```
\
```{r, echo = FALSE, warning = FALSE}
xfun::embed_files('Blog.age.csv', text = "Age data")
```
\
```{r, echo = FALSE, warning = FALSE}
xfun::embed_files('Blog.Transport.csv', text = "Transport employment data")
```
